...
* Data Acquisition: The project describes how the data was obtained and gives substantial background on the data. 
...
he said aloud in class groups of 2-3 are encouraged
written part on markdown
nov 17th til end of class is in class final worktime
## Place Work Below!!
...
```{r}
df <- read.csv("DatingAppReviewsDataset.csv")
...
df <- df |>
  rename("NumThumbsUp" = X.ThumbsUp)


# example on using AFINN --> gotta tokenize the senetences into individual words
...
# apparently afinn cannot handle reviews very well because it only contains about 2,500 words in its lexicon
#chatGPT says Vader is a better option and can handle emojis, slang, short sentences like "can't find"
